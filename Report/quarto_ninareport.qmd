...
The main objective of this review is to map the available literature on effects of managed harvest/hunting on galliform (Norwegian: *Hønsefugl*) species.
...
included_full_details |>
  ...
  mutate(name = fct_relevel(study_extent_size_of_the_study_area, 
                            "Multinational", "National", "Regional (e.g. a county, a larger mountain area etc)", "Local (e.g. a municipality or part of municipality)", "Site level (< 1km2)")) |>
...
In agreement with previous assessment of study designs in ecological research (refs), most studies covered in our mapping were observational (106 studies; @fig-Research_type) and with simple (and less robust) study designs (@fig-Study_design). There were only 6 papers using a "Randomised Controlled Trial (RCT)" approach, the study design that leads to the highest level of evidence for causal understanding (@christie2019simple) and these were limited to Norway (5) and the USA (1). The Norwegian articles are all from the same research project and experiment.
...
We also extracted information about to which extent harvest rates (or percentage of the population removed by harvest) were reported in 71 papers. The remining papers did not explicitly report such information (@fig-Harvest_mortality). Moreover, among the papers that reported harvest rate or harvest mortality, only 27 made any assessment to which extent harvest was additive or not (@fig-Harvest_additivity). Note however that this is somewhat ambiguous, and it might depend on how the concept of additivity is defined: although we did not quantify this by direct coding we note that much fewer studies were in fact able to properly document additivity vs compensation via a robust quantitative approach (see also discussion in ref).
...
We also extracted information from each of the eligible studies about the outcome variables that were examined (@fig-EBV_name2), at two different orders (EBV class and EBV names, respectively). In general, there were about equal number of studies focusing on "Species Traits" (67 records) and "Species Populations" (60 records). Note that we did not record this variable for simulation based studies. Separating the EBV classes into EBV names, we found that the most commonly studies outcome variable was "Survival" (67 records), followed by "Population density/abundance/growth\*" (). In addition, several studies focused on effects of "Movement rates" or "Habitat use/selection" (12 and 6 studies, respectively), and 11 studies investiageted effects on "Sex- and age-structure".
